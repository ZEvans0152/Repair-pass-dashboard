...
Deno.serve(async (req) => {
  try {
    ...
    const results = lowBatteryDevices.map((device) => {
      ...
      const deviceId = String(device.device_id || device.id || device.serial_number || 'Unknown');
    ...
    // Delete all existing TrackerStatus records
    const existing = await base44.asServiceRole.entities.TrackerStatus.list('', 10000);
    for (const record of existing) {
      await base44.asServiceRole.entities.TrackerStatus.delete(record.id);
    }

    // Bulk-create new records in batches of 100
    for (let i = 0; i < results.length; i += 100) {
      await base44.asServiceRole.entities.TrackerStatus.bulkCreate(results.slice(i, i + 100));
    }

    return Response.json({ synced: results.length, totalDevices: allDevices.length });
  } catch (error) {
